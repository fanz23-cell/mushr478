...
class ModelPredictivePathIntegral(BaseController):
    ...
    def get_control(self, pose, reference_xytv, _):
        ...
        # and you do not need to weight them anymore. Also, please follow the 
        # algorithm mentioned in writeup to get the control commends.
        # BEGIN QUESTION 4.6
        "*** REPLACE THIS LINE ***"
        raise NotImplementedError
        # END QUESTION 4.6
        # shift control commend one step forward
        action = np.clip(self.init_controls[0, :],self.min_delta, self.max_delta)
        self.init_controls[:,1] = np.roll(self.init_controls[:,1],-1)
        self.init_controls[:,1][-1] = 0
        return action
